...
const Register: React.FC = () => {
    ...
    const loginWithGoogle = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            ...
            try {
                const response = await authService.googleLogin(tokenResponse.access_token, 'access_token', 'register');
                setConfirmModal({
                    isOpen: true,
                    title: '¡Registro Exitoso!',
                    message: t('auth.register.success')?.replace('{name}', response.user.name) || `Registrado como ${response.user.name}`
                });
            } catch (err: any) {
                setErrors({
        ...
        <div className="auth-card">
            ...
            <div className="mb-6 flex justify-center w-full">
                <button
                    ...
                    className="auth-button flex items-center justify-center gap-3 mb-2"
                >
